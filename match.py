...
def LocalMatching_Guide(PointA, EncodeA, TransMat, PointB, EncodeB, A_crop_values_match, B_crop_values_match, W_LocalRegionSize=0.2):
    """
        【专供引导匹配使用, 解决图像尺寸由正方形变成长方形时坐标偏差问题】
        在局部区域内【局部区域是指映射坐标周围的区域，该区域由指定数目的点划定，与四象限和圆域无关】执行点匹配
    """
    # 输入:
    # PointA, PointB: Numpy数据格式
    ...
    # 转变关键点的坐标【从关键点少的向关键点多的方向映射坐标】
    Point_Map = TransformPoint(PointA, TransMat)
    # 修正图像由正方形变成长方形时导致的坐标偏差
    # Point_Map = Point_Map - np.array([A_crop_values_match[1],A_crop_values_match[0]]) + np.array([B_crop_values_match[1],B_crop_values_match[0]])
    # Point_Map = Point_Map + B_crop_values_match
    # 搜索映射坐标周围的关键点索引
    Distance, Index_PointB_Near = NearKeypoint(PointB, Num_NearPoint, Point_Map)  # Num_NearPoint
    # ++映射坐标与其周围关键点的相似性
    # S1: A[B]numpy高级索引: 使用B中的每一行作为索引从A中获取行
...
def DoLocalMatching_Guide(KeypointA, EncodeA, TransMat, KeypointB, EncodeB, A_crop_values_match, B_crop_values_match, W_LocalRegionSize):
    """
        执行局部匹配【专供引导匹配使用, 解决图像尺寸由正方形变成长方形时坐标偏差问题】
    """
    # 初始化数据【判断KeypointA和KeypointB的数目哪个更少】
    if KeypointA.shape[0] <= KeypointB.shape[0]:
        MatchPointA, MatchPointB = LocalMatching_Guide(KeypointA, EncodeA, TransMat, KeypointB, EncodeB, A_crop_values_match, B_crop_values_match, W_LocalRegionSize)
    else:
        MatchPointB, MatchPointA = LocalMatching_Guide(KeypointB, EncodeB, cv2.invertAffineTransform(TransMat), KeypointA, EncodeA, B_crop_values_match, A_crop_values_match, W_LocalRegionSize)  # 逆变换与转置
    return MatchPointA, MatchPointB
